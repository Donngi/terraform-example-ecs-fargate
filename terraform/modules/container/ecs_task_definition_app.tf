...
resource "aws_ecs_task_definition" "app_nginx" {
  ...
  execution_role_arn = aws_iam_role.ecs_task_execution_app.arn
  ...
  container_definitions = jsonencode([
    {
      ...
      "portMappings" : [
        {
          ...
          "containerPort" : 80,
          "protocol" : "tcp"
          # ECS Service Connect(Client-service mode)利用時は、nameの指定が必須。ECS Service側の設定値と合わせる
          # 他のコンテナからは、 {name}.{namespace} でアクセスできる
          "name" : "app-nginx-ecs-service-connect-80-tcp"
        }
      ],
      ...
      "logConfiguration" : {
        ...
        "options" : {
          ...
          "awslogs-group" : aws_cloudwatch_log_group.app_nginx.name,
